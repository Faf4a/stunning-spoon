...
function parseHTMLContent(html, path) {
    ...
    const usageMatch = html.match(/<h2[^>]*>Usage<\/h2>\s*<pre[^>]*>(.*?)<\/pre>/s);
    const usage = usageMatch ? usageMatch[1].trim() : "";
    const exampleMatch = html.match(/<h2>Example\(s\)<\/h2>\s*(?:<blockquote>.*?<\/blockquote>\s*)?(?:<p>.*?<\/p>\s*)*<pre><code[^>]*>(.*?)<\/code><\/pre>(?:\s*<p>.*?<\/p>\s*<pre><code[^>]*>(.*?)<\/code><\/pre>)*/gs);

    const example = exampleMatch ? exampleMatch[0].replace("<h2>Example(s)</h2>", "") : "";
    ...
    const documentation = ["aoi.music", "aoi.canvas"].includes(packageName)
        ? `https://aoi.js.org/extensions/${packageName}/${functionName.toLowerCase().replace("$", "")}`
        : `https://aoi.js.org/functions/${functionName.toLowerCase().replace("$", "")}`;

    const sourceCode = ["aoi.music", "aoi.canvas"].includes(packageName)
        ? `https://github.com/aoijs/${packageName}`
        : `https://github.com/aoijs/aoi.js/tree/v6/src/functions/${basename(path, extname(path))}.js`;
...
function extractTable(html) {
    ...
    const headers = headersMatch.map((header) => header.replace(/<\/?th[^>]*>/g, "").trim());
        ...
        if (statSync(folderPath).isDirectory() && packages[folder]) {
            ...
            files.forEach((file) => {
                if (extname(file) === ".md" || extname(file) === ".mdx") {
                    ...
                    const { result: html, duration: renderDuration } = measureExecutionTime(md.render.bind(md), content);
                    const { result: object, duration: parseDuration } = measureExecutionTime(parseHTMLContent, html, path);
...
const { result: docsResult, duration: parseFilesDuration } = measureExecutionTime(parseFiles, docs);
writeFileSync("functions.json", JSON.stringify(docsResult, null, 2));

console.table(
    docsResult.map(({ function: functionName, renderDuration, parseDuration }) => ({
